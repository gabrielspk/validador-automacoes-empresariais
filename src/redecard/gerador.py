...
def gerar_relatorio(df):
    ...
    df = df.loc[~df['Processo'].str.contains(r"redecard_envioarscliente_rem", na=False)]
    
    originais = df[df['Nome Arquivo'].str.contains('import_padrao_3604', na=False) & ~df['Status'].str.contains('Corrigido|Erro', na=False)]
    corrigidos = df[df['Nome Arquivo'].str.contains('import_padrao_3604', na=False) & df['Status'].str.contains('Corrigido', na=False)]
    erros = df[df['Nome Arquivo'].str.contains('import_padrao_3604', na=False) & df['Status'].str.contains('Erro', na=False)]
    parados = df[df['Nome Arquivo'].str.contains('import_padrao_3604', na=False) & df['Status'].str.contains('Parado', na=False)]
    entregar = df[df['Nome Arquivo'].str.contains('import_padrao_3604', na=False) & df['Status'].str.contains('Entregar', na=False)]
