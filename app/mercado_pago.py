...
def executar():
    ...
    print("\n🟢 Iniciando processamento: MERCADO PAGO")

    cliente = "mercado_pago"
    ...
    baixar_planilha(driver, "mercado pago")
    driver.quit()
    ...
    if df is None:
        print("❌ Nenhum dado carregado para Mercado Pago.")
        return
    ...
    assunto = (
        f"⚠️ VALIDAÇÃO MERCADO PAGO {data_anterior_formatada} e {data_atual_formatada} - VERIFICAR DIVERGÊNCIAS ENCONTRADAS"
        if divergencias else
        f"✅ VALIDAÇÃO MERCADO PAGO {data_anterior_formatada} e {data_atual_formatada} - VALIDADO COM SUCESSO"
    )
