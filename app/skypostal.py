...
from src.skypostal.gerador import gerar_relatorio, salvar_relatorio_com_validacao
from src.skypostal.validador import validar_registros
...
def executar():
    ...
    print("\n🟢 Iniciando processamento: SKYPOSTAL")
    ...
    baixar_planilha(driver, "skypostal")
    driver.quit()
    ...
    if df is None:
        print("❌ Nenhum dado carregado para skypostal.")
        return
    ...
    assunto = (
        f"⚠️ VALIDAÇÃO SKYPOSTAL {data_anterior_formatada} e {data_atual_formatada} - VERIFICAR DIVERGÊNCIAS ENCONTRADAS"
        if divergencias else
        f"✅ VALIDAÇÃO SKYPOSTAL {data_anterior_formatada} e {data_atual_formatada} - VALIDADO COM SUCESSO"
    )
